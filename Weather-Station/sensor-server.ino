...
It is STRONGLY reccomended the APSSID and APPSK variubles are updated with new credentials dreamed up by you.
...
- to view AP ssid, password, and starting IP address upon startup the console port will output this when the device is powered on and is set to BAUD 115200, which means that it is reccomended that the device
  is not accessable to attackers or is enclosed in a case that does not allow for easy access to the console port.

## TODO: 
  - allow for configuration of the SSID and Password through a webpage
  - add compatability for connecting to a network and sending data to a collection server 
